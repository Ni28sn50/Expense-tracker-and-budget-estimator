...
def firstwin():
    ...
    Button(f1, text="Login", cursor="hand2", command=lambda: login(uname.get(), password.get()), image=login_but,
           bg="black", borderwidth=0, highlightthickness=0).place(x=425, y=380)
    Button(f1, text="Reset", cursor="hand2", command=lambda: clear(uname, password), image=reset_but, bg="black",
           borderwidth=0, highlightthickness=0).place(x=575, y=380)
    Button(f1, text="Don't have an account?", cursor="hand2", command=create_account, fg='#89CFF0', bg="black",
           borderwidth=0, highlightthickness=0, font=font2 + ('underline',)).place(x=420, y=460)
    cvar = IntVar()
    cvar.set(2)
    win1.mainloop()
...
def create_account():
    ...
    Button(f4, cursor="hand2", command=lambda: chk_account(name.get(), pswd.get(), con_pswd.get()),
           text="Create account", fg='white', bg='#3895D3', border=4, font=("Roboto Mono", 14)).place(x=340, y=380)
    win3.mainloop()
...
def chk_account(name, pswd, con_pswd):
    if (len(name) == 0) or (len(pswd) == 0):
        ...
        if len(list_of_names) == 0:
            messagebox.showinfo("Accout Creation Successful",
                                "Account has been successfully created. Now you may close the Account Creation window.")
            cur.execute("INSERT INTO Login(name, password) VALUES(?, ?)", (name, pswd))
            conn.commit()
        else:
            messagebox.showerror("User Name already taken",
                                 "This User Name had already been taken, please try with a different User Name.")
...
def chk_rec(item, cost):
    if (len(item)) == 0 or (len(cost)) == 0:
        ...
        if cost < 0:
            ...
            mon_spent = cur.execute("SELECT moneyspent FROM MonthlyExp WHERE user_id=? AND item_id=?",
                                    (user_id[0][0], item_id[0][0])).fetchall()
            if (len(mon_spent) > 0):
                cur.execute("UPDATE MonthlyExp SET moneyspent=? WHERE user_id=? AND item_id=?",
                            (sum(mon_spent[0] + (cost,)), user_id[0][0], item_id[0][0]))
            else:
                cur.execute("INSERT INTO MonthlyExp(user_id, item_id, moneyspent) VALUES(?, ?, ?)",
                            (user_id[0][0], item_id[0][0], cost))
...
def addrec(addrec_button):
    ...
    Button(f3, cursor="hand2", command=lambda: chk_rec(item_name.get(), cost.get()), text="Add Record", fg='white',
           bg='#3895D3', border=4, font=("Roboto Mono", 14)).place(x=260, y=320)
    Button(f3, cursor="hand2", command=lambda: clear(item_name, cost), text="Reset", fg='white', bg='#3895D3', border=4,
           width=10, font=("Roboto Mono", 14)).place(x=420, y=320)
...
def showall(showall_button):
    ...
    if (chk[0][0] == 0):
        ...
        table = cur.execute(
            "SELECT Items.item, MonthlyExp.moneyspent FROM Items JOIN MonthlyExp ON Items.id=MonthlyExp.item_id WHERE MonthlyExp.user_id=?",
            (user_id[0][0],)).fetchall()
        style = ttk.Style()
        ...
        inst_label = Label(f3,
                           text="(If number of items are more than 10, then you can either scroll down on the table or\nselect last row and press down key to view remaining items.)",
        ...
        sum_label = Label(f3, text=f"Total Money Spent: Rs. {sum_exp[0][0]}", font=("Times", 20, "bold"), bg="Black",
                          fg="White")
...
def edit(item_name, updt_item_name, updt_cost):
    if (len(item_name) == 0):
        messagebox.showerror("No Item Name given",
                             "You left the field in which you have to enter item name whose details are to be updated!")
    else:
        ...
        if (len(item_id) < 1):
            ...
            chk = cur.execute("SELECT EXISTS(SELECT * FROM MonthlyExp WHERE user_id=? AND item_id=?)",
                              (user_id[0][0], item_id[0][0],)).fetchall()
            if (chk[0][0] == 0):
                messagebox.showerror("Error", "The entered item is not present in the record.")
            else:
                if (len(updt_item_name) == 0) and (len(updt_cost) == 0):
                    messagebox.showerror("Empty Fields",
                                         "Please fill at least one field among Updated Item name and Updated Cost.")

                elif len(updt_item_name) == 0:
                    updt_cost = chk_cost(updt_cost)
                    if updt_cost < 0:
                        messagebox.showerror("Invalid value of Updated Cost",
                                             "Please enter valid value of Updated Cost.")
                    else:
                        cur.execute("UPDATE MonthlyExp SET moneyspent=? WHERE user_id=? AND item_id=?",
                                    (updt_cost, user_id[0][0], item_id[0][0]))
                    ...
                    if (len(item_id2) < 1):
                        ...
                        item_id2 = cur.execute("SELECT id FROM Items WHERE item=?",
                                               (updt_item_name.upper(),)).fetchall()
                        cur.execute("UPDATE MonthlyExp SET item_id=? WHERE user_id=? AND item_id=?",
                                    (item_id2[0][0], user_id[0][0], item_id[0][0]))
                    else:
                        chk2 = cur.execute("SELECT EXISTS(SELECT * FROM MonthlyExp WHERE user_id=? AND item_id=?)",
                                           (user_id[0][0], item_id2[0][0])).fetchall()
                        if (chk2[0][0] == 0):
                            cur.execute("UPDATE MonthlyExp SET item_id=? WHERE user_id=? AND item_id=?",
                                        (item_id2[0][0], user_id[0][0], item_id[0][0]))
                        else:
                            cost1 = cur.execute("SELECT moneyspent FROM MonthlyExp WHERE user_id=? AND item_id=?",
                                                (user_id[0][0], item_id[0][0])).fetchall()
                            cost2 = cur.execute("SELECT moneyspent FROM MonthlyExp WHERE user_id=? AND item_id=?",
                                                (user_id[0][0], item_id2[0][0])).fetchall()
                            cur.execute("UPDATE MonthlyExp SET moneyspent=? WHERE user_id=? AND item_id=?",
                                        (sum(cost1[0] + cost2[0]), user_id[0][0], item_id2[0][0]))
                            cur.execute("DELETE FROM MonthlyExp WHERE user_id=? AND item_id=?",
                                        (user_id[0][0], item_id[0][0]))
                    messagebox.showinfo("Updation Successful", "Record has been updated successfully!")
                    ...
                    if updt_cost < 0:
                        ...
                        if (len(item_id2) < 1):
                            ...
                            item_id2 = cur.execute("SELECT id FROM Items WHERE item=?",
                                                   (updt_item_name.upper(),)).fetchall()
                            cur.execute("UPDATE MonthlyExp SET item_id=?, moneyspent=? WHERE user_id=? AND item_id=?",
                                        (item_id2[0][0], updt_cost, user_id[0][0], item_id[0][0]))
                        else:
                            chk2 = cur.execute("SELECT EXISTS(SELECT * FROM MonthlyExp WHERE user_id=? AND item_id=?)",
                                               (user_id[0][0], item_id2[0][0])).fetchall()
                            if (chk2[0][0] == 0):
                                cur.execute(
                                    "UPDATE MonthlyExp SET item_id=?, moneyspent=? WHERE user_id=? AND item_id=?",
                                    (item_id2[0][0], updt_cost, user_id[0][0], item_id[0][0]))
                            else:
                                cost2 = cur.execute("SELECT moneyspent FROM MonthlyExp WHERE user_id=? AND item_id=?",
                                                    (user_id[0][0], item_id2[0][0])).fetchall()
                                cur.execute("UPDATE MonthlyExp SET moneyspent=? WHERE user_id=? AND item_id=?",
                                            (sum(cost2[0] + (updt_cost,)), user_id[0][0], item_id2[0][0]))
                                cur.execute("DELETE FROM MonthlyExp WHERE user_id=? AND item_id=?",
                                            (user_id[0][0], item_id[0][0]))
                    messagebox.showinfo("Updation Successful", "Record has been updated successfully!")
            conn.commit()
...
def editrec(editrec_button):
    ...
    updt_cost = StringVar()
    edit_label = Label(f3, text="Enter Item name whose details you want to edit:-", font=("Times", 20), bg="Black",
                       fg="White")
    ...
    inst_label = Label(f3, text="(Leave that field empty in which you don't want any updation)", font=("Times", 18),
                       bg="Black", fg="White")
    ...
    Button(f3, cursor="hand2", command=lambda: edit(item_name.get(), updt_item_name.get(), updt_cost.get()),
           text="Confirm Changes", fg='white', bg='#3895D3', border=4, font=("Roboto Mono", 14)).place(x=100, y=480)
    Button(f3, cursor="hand2", command=lambda: clear(updt_item_name, updt_cost), text="Reset values in Updated fields",
           fg='white', bg='#3895D3', border=4, font=("Roboto Mono", 14)).place(x=350, y=480)
...
def confirmdelrec(delitem):
    ...
    if (len(item_id) == 0):
        ...
        if (chk[0][0] == 0):
            ...
            res = messagebox.askquestion("Confirm Deletion",
                                         "The record of the entered item will be deleted permanently from the record. Do you wish to continue?")
            if res == 'yes':
...
def delrec(delrec_button):
    ...
    del_heading = Label(f3, text="Enter Item name whose details you want to delete:-", font=("Times", 20), bg="Black",
                        fg="White")
    ...
    Button(f3, cursor="hand2", command=lambda: confirmdelrec(delitem_name.get()), text="Confirm Deletion", fg='white',
           bg='#3895D3', border=4, font=("Roboto Mono", 14)).place(x=250, y=250)


def confirmdelall(password):
    chk = cur.execute("SELECT EXISTS(SELECT * FROM Login WHERE id=? AND password=?)",
                      (user_id[0][0], password)).fetchall()
    if chk[0][0] == 0:
...
def delallrec(delallrec_button):
    ...
    password = StringVar()
    warn_label = Label(f3,
                       text="WARNING: You are about to delete all of your records.\nIf you wish to continue, then please enter your password.",
    ...
    Button(f3, cursor="hand2", command=lambda: confirmdelall(password.get()), text="Confirm Deletion", fg='white',
           bg='#3895D3', border=4, font=("Roboto Mono", 14)).place(x=250, y=250)
...
def secwin():
    ...
    l4.place(x=100, y=0)
    btn_set = set()
    addrec_button = Button(f2, text="Add a record", cursor="hand2", command=lambda: addrec(addrec_button), border=4,
                           width=37, height=3)
    addrec_button.place(x=10, y=60)
    btn_set.add(addrec_button)
    showall_button = Button(f2, text="Show all records and Total Money Spent", cursor="hand2",
                            command=lambda: showall(showall_button), border=4, width=37, height=3)
    showall_button.place(x=10, y=160)
    btn_set.add(showall_button)
    editrec_button = Button(f2, text="Edit a record", cursor="hand2", command=lambda: editrec(editrec_button), border=4,
                            width=37, height=3)
    editrec_button.place(x=10, y=260)
    btn_set.add(editrec_button)
    delrec_button = Button(f2, text="Delete a record", cursor="hand2", command=lambda: delrec(delrec_button), border=4,
                           width=37, height=3)
    delrec_button.place(x=10, y=360)
    btn_set.add(delrec_button)
    delallrec_button = Button(f2, text="Delete all records", cursor="hand2",
                              command=lambda: delallrec(delallrec_button), border=4, width=37, height=3)
